...
#if defined(NDEBUG)
# error "KudosAlpha cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x9c4d942cf816d04475a4a178fd6327e3a83ebdb45f9d06f94b319b58ee42a399");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // KudosAlpha: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "KudosAlpha Signed Message:\n";
...
{
    // KudosAlpha: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // KudosAlpha: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // KudosAlpha: 1 days
static const int64 nTargetSpacing = 2 * 60; // KudosAlpha: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // KudosAlpha: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // KudosAlpha: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x13ca1b8690857879b0aa9835be4f64c8329c4da76bfaadc8e694338de558fc7a");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Day after Scotland rejected independence";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1411116882;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1445274;

        if (fTestNet)
        {
            block.nTime    = 1410703206;
            block.nNonce   = 294466;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x8d7b9fceacdde1be13019611bab3c7826f0b2cdea2e410959cf87fe2c86e846d"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // KudosAlpha: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KudosAlphaMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KudosAlphaMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KudosAlphaMiner(CWallet *pwallet)
{
    printf("KudosAlphaMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("kudosalpha-miner");
    ...
    try { loop {
        ...
        printf("Running KudosAlphaMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("KudosAlphaMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KudosAlphaMiner, pwallet));
}
